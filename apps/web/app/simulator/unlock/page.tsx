...
export default function SimulatorUnlockPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    setErr(false);
    const r = await fetch("/api/sim/login", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ password: pw }),
    });
    if (r.ok) {
      // Full navigation — /simulator is a standalone HTML document, not a React route.
  ...
  return (
    <div style={{ minHeight: "72vh", display: "grid", placeItems: "center" }}>
      <form onSubmit={submit} className="card" style={{ width: "min(360px, 92vw)", padding: 22 }}>
        <div className="brand" style={{ marginBottom: 6, fontSize: 19 }}>
          <span className="dot" />
          DFS Simulator
        </div>
        <div className="muted" style={{ fontSize: 12.5, marginBottom: 16 }}>Owner-only. Enter the simulator password.</div>
